...
export default function FitFactorPaymentPage() {
  ...
  const unitPriceSGD = 30; 
  const bundlePriceSGD = 50; 
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`payment-status-fitfactor-${paymentData.tripay_reference}`)
  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="p-6 flex justify-between items-center">
        ...
        <h1 className="text-xl font-bold">Checkout FitFactor</h1>
        <div className="flex bg-secondary p-1 rounded-lg">
          {['SGD', 'USD', 'MYR'].map((cur) => (
            <button key={cur} onClick={() => setCurrency(cur)} className={`px-3 py-1 text-xs font-bold rounded transition-all ${currency === cur ? 'bg-gradient-to-r from-[#BF953F] via-[#FCF6BA] to-[#B38728] text-amber-950' : 'text-muted-foreground'}`}>{cur}</button>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="text-xs text-blue-600 bg-blue-50 p-2 rounded">{quantity >= 3 ? "✨ Bundle Promo & FREE Shipping applied!" : "Add 3 for Bundle Promo & FREE Shipping"}</div>
            <Separator />
        ...
        <div className="fixed bottom-20 left-6 right-6">
          <Button onClick={handleCreatePayment} disabled={loading} className="w-full h-14 text-xl font-bold bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
