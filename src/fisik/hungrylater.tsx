...
export default function HungryLaterPaymentPage() {
  ...
  const productName = 'HungryLater Weight Control';
  const unitPriceSGD = 35; 
  const bundlePriceSGD = 90; 
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke('tripay-create-payment', {
        body: {
          ...
          amount: totalAmount,
          currency, quantity,
          productName: `${productName} (${quantity} Boxes)`,
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`payment-status-hungrylater-${paymentData.tripay_reference}`)
  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="p-6 flex justify-between items-center">
        ...
        <h1 className="text-xl font-bold">Checkout HungryLater</h1>
      ...
      <div className="px-6 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="text-xs text-orange-600 bg-orange-50 p-2 rounded">{quantity >= 3 ? "✨ Bundle Promo & FREE Shipping applied!" : "Add 3 for Bundle Promo & FREE Shipping"}</div>
            <Separator />
        ...
        <div className="fixed bottom-20 left-6 right-6">
          <Button onClick={handleCreatePayment} disabled={loading} className="w-full h-14 text-xl font-bold bg-gradient-to-r from-orange-500 to-red-600 text-white">
